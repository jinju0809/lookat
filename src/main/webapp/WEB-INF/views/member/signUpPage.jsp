...
<script type="text/javascript">
	$(document).ready(function() {
 	...
 	function fn_idCheck() {
		$('#m_id').keyup(function() {
			...
			if (regId.test($('#m_id').val())) { // 정규식 패턴을 만족한다.
				$.ajax({
					url : 'idCheck.do',
					type : 'post',
	...
	function fn_allCheck() {
		$('#provisionTable').on('click', '#allCheck', function() {	
/* 			var checked = $(this).is(":checked");
			if (checked) {
				$(this).parents('#provisionTable').find('input').prop("checked", true);
			} else {
				$(this).parents('#provisionTable').find('input').prop("checked", false);
			} */
			$(this).parents('#provisionTable').find('input').prop("checked", $(this).is(":checked"));
		});
	}
	...
	function fn_register() {
		$('#register_btn').on('click', function() {
			...
			$.ajax({
				url : 'register.do',
				type : 'post',
				data : JSON.stringify(sendInsert), // JSON -> String 형변환 후 컨트롤러로 보내는 String 타입의 JSON 데이터
				contentType : 'application/json', // 컨트롤러로 보내는 데이터의 타입을 알려주기 위해
				dataType : 'json',
				success : function(responseObj) { // 성공하면 받아오는 데이터
					if (responseObj.registerResult == 0) {
						alert('아이디 항목은 필수 입력값입니다.')
						$('#m_id').focus();
					} else if (responseObj.registerResult == -1) {
						alert(m_id + '는 이미 사용중인 아이디입니다.')
						$('#m_id').focus();
					} else if (responseObj.registerResult == -2) {
						alert('아이디는 소문자로 시작, 소문자 또는 숫자 5~20자로 입력해 주세요.')
						$('#m_id').focus();
					} else if (responseObj.registerResult == -3) {
						alert('비밀번호 항목은 필수 입력값입니다.')
						$('#m_pw').focus();
					} else if (responseObj.registerResult == -4) {
						alert('비밀번호 입력 조건을 다시 한번 확인해주세요.\n\n'
								...
								+ '   ` ~ ! @ # $ % ^ & * ( ) - _ = +')
						$('#m_pw').focus();
					} else if (responseObj.registerResult == -5) {
						alert('비밀번호가 일치하지 않습니다.')
						$('#m_pw2').focus();
					} else if (responseObj.registerResult == -6) {
						alert('이름 항목은 필수 입력값입니다.')
						$('#m_name').focus();
					} else if (responseObj.registerResult == -7) {
						alert('휴대전화를 입력해주세요.');
						$('#m_phone2').focus();
					} else if (responseObj.registerResult == -8) {
						alert('이메일을 입력하세요.');
						$('#m_email1').focus();
					} else if (responseObj.registerResult == -9) {
						alert(m_email + '는 이미 사용중인 이메일입니다.');
						$('#m_email1').focus();
					} else if (responseObj.registerResult == -10) {
						alert('입력하신 이메일을 사용할 수 없습니다.');
						$('#m_email1').focus();
					} else if (responseObj.registerResult == -11) {
						alert('이용약관에 동의하세요.');
					} else if (responseObj.registerResult == -12) {
						alert('이용약관에 동의하세요.');
					} else if (responseObj.registerResult == 1) {
						alert(m_id + '님 환영합니다.');
						location.href = 'index.do';
					} else {
						alert('registerResult == null');
					}
				},
				error : function() {
					alert('AJAX FAIL')
...
#informationTable>tbody>tr>td>.information {
	border: 1px solid lightgrey;
}

#provisionTable, #informationTable {
	width: 100%;
}
...
<br/><br/><br/><br/><br/>
<p style="color: grey;  font-size: 14px; text-align: center;">MEMBERSHIP</p>
...
<tbody>
	<tr>
		...
		<td>
			<input class="information" type="text" id="m_id" name="m_id" />
			<span id="idCheckResult">(소문자로 시작, 소문자나 숫자 사용, 5~20자)</span>
		</td>
	</tr>
	<tr>
		<td>비밀번호 <span class="essential">*</span></td>
		<td>
			<input class="information" type="password" id="m_pw" name="m_pw" />
			<span id="pwCheckResult">(영문 대소문자/숫자 조합, 8~16자)</span>
		</td>
	</tr>
	<tr>
		<td>비밀번호 확인 <span class="essential">*</span></td>
		<td>
			<input class="information" type="password" id="m_pw2" name="m_pw2" />
	...
	<tr>
		...
		<td>
			<input class="information" type="text" id="m_name" name="m_name" />
		</td>
	</tr>
	<tr>
		<td>휴대전화 <span class="essential">*</span></td>
		<td>
			<select class="information" id="m_phone1">
			...
			-
			<input style="width: 50px" class="information" type="text" id="m_phone2" />
			-
			<input style="width: 50px" class="information" type="text" id="m_phone3" />
		</td>
	</tr>
	<tr>
		<td>이메일 <span class="essential">*</span></td>
		<td>
			<input class="information" type="text" id="m_email1" />
			@
			<input class="information" id="m_email2" placeholder="이메일을 선택하세요.">
			<select class="information" id="selectEmail" >
...
<p style="font-size: 12px; font-weight: 700;">전체 동의</p>
<table id="provisionTable">
	<tbody>
		<tr>
		<td><input type="checkbox" id="allCheck">&nbsp;<label style="font-size:12px;font-weight: bold" for="allCheck">이용약관 및 개인정보수집 및 이용, 쇼핑정보 수신(선택)에 모두 동의합니다.</label></td>
		</tr>
		<tr>
			<td>
			[필수] 이용약관 동의<br/>
...
부 칙(시행일) 이 약관은 년 월 일부터 시행합니다.
			...
			이용약관에 동의하십니까? <input id="check1" type="checkbox"/>&nbsp;<label for="check1">동의함</label>
			</td>
		</tr>
		<tr>
			<td>
...
※ 동의를 거부할 수 있으나 거부시 회원 가입이 불가능합니다.
				...
				개인정보 수집 및 이용에 동의하십니까? <input id="check2" type="checkbox"/>&nbsp;<label for="check2">동의함</label>
...
선택 약관에 동의하지 않으셔도 회원가입은 가능하며, 회원가입 후 회원정보수정 페이지에서 언제든지 수신여부를 변경하실 수 있습니다.
				...
				이메일 수신을 동의하십니까? <input id="check3" type="checkbox"/>&nbsp;<label for="check3">동의함</label>
...
<br/>
<div id="register_btn_Container"><button id="register_btn">회원가입</button></div>
</div>
