...
public class MemberEmailAuthConfirmCommand implements MemberCommand {
	...
	public Map<String, Object> execute(SqlSession sqlSession, Model model) {
		...
		String authKey = memberDto.getAuthKey();
		String userKey = memberDto.getUserKey();

		if (userKey.equals("")) {
			resultMap.put("emailAuthConfirmResult", 0);
		} else if (!(authKey.equals(userKey))) {
			resultMap.put("emailAuthConfirmResult", -1);
		} else if (authKey.equals(userKey)) {
			...
			try {
				...
				memberDao.findPassword(m_id, m_name, m_email);
				String m_pw = Sha256.sha256(temporaryPw);
				memberDao.changePassword(m_id, m_pw);
				resultMap.put("temporaryPw", temporaryPw);
				resultMap.put("emailAuthConfirmResult", 1);
			...
			resultMap.put("emailAuthConfirmResult", -2);
		}
